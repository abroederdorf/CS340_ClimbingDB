		...
		<div id="pageDiv">
			...
			<p>
				<?php	
					...
					if(!($stmt = $mysqli->prepare("INSERT INTO mtn_route(name, mtid) VALUES (?,?)"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!($stmt->bind_param("si",$_POST['rname'],$_POST['mtn']))){
					...
					if(!$stmt->execute()){
						...
						if ($stmt->affected_rows == 0)
							echo "Error, added " . $stmt->affected_rows . " rows to mtn_route.";
						else
							echo "Added " . $stmt->affected_rows . " row to mtn_route.";
					}
				?>
			</p>
