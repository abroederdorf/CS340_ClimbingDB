		...
		<div id="pageDiv">
			...
			<p>
				<?php	
					...
					if(!($stmt = $mysqli->prepare("INSERT INTO mtn_climberSkill(cid, sid) VALUES (?,?)"))){
						echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
					}
					if(!($stmt->bind_param("ii",$_POST['climberCliSk'],$_POST['skillCliSk']))){
					...
					if(!$stmt->execute()){
						...
						if ($stmt->affected_rows == 0)
							echo "Error, added " . $stmt->affected_rows . " rows to mtn_climberSkill.";
						else
							echo "Added " . $stmt->affected_rows . " row to mtn_climberSkill.";
					}
				?>
			</p>
